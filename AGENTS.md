...
- One **connector-grain** asset, id still `groupId.schema` (falling back to `connectorId.schema` when `groupId` is absent), so upgrading renames nothing and connector-level freshness keeps somewhere to live. Typed `Dataset`, not `Table`, because it covers a whole destination schema rather than one table. Both types have existed since EE 1.2.0.
